...
app.get("/admin/api/actives", async function (req, res, next) {
  try {
    console.log("api1");
    const session = req.session;
    let listViewModel = require("../../view_models/actives_admin_list_paginate_view_model");
    ...
    const { rows: allItems, count } = await db.active.findAndCountAll({
      where: where,
