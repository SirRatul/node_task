...
$(document).on("click", ".selectionBtn", function (evt, isCustom) {
  ...
  if (["Banana", "Olive", "Sunflowers"].indexOf(val) !== -1) {
    terminateQuiz();
  } else if (!isCustom) {
    timeout = setTimeout(function () {
      nextQuestion();
...
function checkAllergie() {
  return new Promise(function (resolve, reject) {
    $.ajax({
      url: url_preset + "/admin/api/actives",
      type: "GET",
      success: function (response) {
        var responseData = response.data; //all data in the api
        var activeItems = responseData.items; // items in the api
        activeItems.forEach((val, index) => {
          $("#typeSelection .answerInner").append(`<div class="selectionOptions"><button data-val="${val.name}" data-id="${val.id}" class="selectionBtns selectionBtn">${val.name}</button></div>`);
        });
        return resolve();
      },
      error: function (error) {
...
function terminateQuiz() {
  ...
  var timer = setInterval(function () {
    ...
    document.body.innerHTML = `<div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);; color: white; text-align: center;"><p>You have an allergy to one of the main ingredients in our system. Our current system will not suit you</p><h2 style="font-style: italic;">Quiz will now be terminated</h2><p style="font-style: italic;">${timeleft}</p></div>`;
    if (timeleft < 0) {
